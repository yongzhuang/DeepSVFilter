...
class model(object):
    ...
    def train(self, train_im, train_label, eval_im, eval_label, batch_size, patch_size, epoch, eval_every_epoch, lr,
        ...
        for epoch in range(start_epoch, epoch):  # epoch=20
            ...
            if (int(epoch) + 1) % int(eval_every_epoch) == 0:
                self.evaluate(epoch + 1, eval_im, eval_label, batch_size, patch_size, iter_num, epoch)  # Eval set test
    ...
    def test(self, test_im, test_label, test_names, patch_size, ckpt_dir, res_dir):
        ...
        batch_input_y = np.zeros((len(test_im), 2), dtype="float32")
        image_id = 0
        for patch_id in range(len(test_im)):  # 16
            batch_input_im[patch_id, :, :, :] = test_im[image_id]
            batch_input_y[patch_id, :] = test_label[image_id]

            image_id = (image_id + 1) % len(test_im)
            if image_id == 0:
                tmp = list(zip(test_im, test_label))
                random.shuffle(list(tmp))
                test_im, test_label = zip(*tmp)

        test_predict_label, test_predict_loss, test_accuracy, input_classes, logits = self.sess.run(
            [self.prediction, self.loss_SV, self.accuracy, self.input_classes, self.logits],
            feed_dict={self.input_im: batch_input_im,
                       self.input_label: batch_input_y,
                       self._keep_prob: 1})
        ...
        input_classes = [str(a) for a in input_classes.tolist()]
        test_predict_label = [str(a) for a in test_predict_label.tolist()]

        count_tp, count_tn, count_fn, count_fp, \
        tpr, fpr, tnr, acc, precision, recall, f1, roc_auc = evaluation1(true_label, predict_score,
                                                                input_classes, test_predict_label,
                                                                self._model_name, _global_step, res_dir)
        print(
            "Test_predict_loss=%.6f, AUC=%.4f, TPR=%.4f, FPR=%.4f, Test_accuracy=%.4f, TNR=%.4f, Precision=%.4f, Recall=%.4f, F1-score=%.4f" % (
                test_predict_loss, roc_auc, tpr, fpr, test_accuracy, tnr, precision, recall, f1))
